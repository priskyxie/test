...
logfile = "docker_pytorch_"+str(time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime())) +".log"
logconf = {
    'version': 1,
...
class SPARSENN():
    ...
    def gen_execfile(self, spnn_exec):
        with open(spnn_exec, "w") as f:
            ...
            f.write("\n")
            f.write("HIP_VISIBLE_DEVICES=0,1,2,3")
            f.write("\n")
...
if __name__ == "__main__":
    ...
    ### BKC 2097
    #DOCKER_IMAGE = "56d7bff9ed0e"
    DOCKER_IMAGE = "rocm/pytorch:rocm3.1_ubuntu16.04_py3.6_caffe2"
    EXEC_SCRIPT_PATH = "/usr/bin/python3.6 /root/dlrm/dlrm_s_pytorch.py"
    embedding_size = "1000000-" * 31 + "1000000"
    sparse_feature_size = "32"
    PARAMETERS = r'\--arch-sparse-feature-size %s \
        --arch-embedding-size %s\
        --arch-mlp-bot "512-512-512-%s" \
        ...
        --nepochs 1 \
        --use-gpu \
        --print-time' % (sparse_feature_size, embedding_size, sparse_feature_size)

    sp = SPARSENN(DOCKER_IMAGE, EXEC_SCRIPT_PATH, PARAMETERS)
    sp.run()
