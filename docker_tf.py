...
logfile = "docker_TF_"+str(time.strftime("%Y-%m-%d_%H-%M-%S", time.localtime())) +".log"
logconf = {
    'version': 1,
    ...
    with p.stdout:
        ...
        for line in iter(p.stdout.readline, b''):
           if "VendorID" in str(line) and "DeviceID" in str(line) and "SSID" in str(line):
              atitool_gpu_ids.append(str(line).strip().split()[0])
...
class CAFFE2():
    ...
    def gen_dockerfile(self, dockerfile):
        work_dir = self.exec_script_path.split()[1].split("/convnet_benchmarks.py")[0]
        with open(dockerfile, "w") as f:
            f.write("FROM %s" % self.docker_image)
            f.write("\n")
            f.write("WORKDIR %s" % work_dir)
...
if __name__ == "__main__":
    ### alexnet 4p 2gbps = 1869 images/s
    ### single 1p resnet50 = 92 images/s =>1719s/1000 batches
    ### single
    ### 4GPUs resnet50 and batch_size=128, images / sec: 413.28     1000batch=1777second
    ## ========= run Tensorflow benchmark ==============
    #DOCKER_IMAGE = "rocmqa/tf1.15:1901_MIOpen2.2.x-py3-ub18.04"

    ### bkc 2097 image
    DOCKER_IMAGE = "72594318997e"
    EXEC_SCRIPT_PATH = "python3 /root/benchmarks/scripts/tf_cnn_benchmarks/tf_cnn_benchmarks.py"
    PARAMETERS = "--model=resnet50 --num_batches=10000 --num_gpus=4 --batch_size=128 --train_dir=/results --save_summaries_steps 10 --save_model_secs=3600 --print_training_accuracy=True"
    tf = TENSORFLOW(DOCKER_IMAGE, EXEC_SCRIPT_PATH, PARAMETERS)
    tf.run()
